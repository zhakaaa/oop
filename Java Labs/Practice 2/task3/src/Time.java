class Time implements Comparable<Time> {
    ...
    public int compareTo(Time time) {
        if(this.hour > time.hour && this.minute > time.minute && this.second > time.second) return 1;
        else if(this.hour < time.hour && this.minute < time.minute && this.second < time.second) return -1;
        else return 0;
    }
